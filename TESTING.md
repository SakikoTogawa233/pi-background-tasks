...
`test:tmux` is the required real-Pi TUI gate on Linux hosts with `/usr/bin/tmux`. It does not skip and is independent of the Expect capability probe.

Windows process-tree integration is a separate platform gate and is not included in `test:full`:
...
### Foreground bash and adopted-task gates

The foreground fallback is intentionally covered at each boundary where ownership can be lost:

- `tests/unit/foreground-bash.test.ts` pins the 2-second fast-path/hint timers, 120-second total-runtime threshold and per-call override, fast completion, manual `Ctrl+B`, newest-call selection, non-interactive behavior, sleep guard, streaming/truncation, abort/adoption races, ownership transfer, and registration.
- `tests/unit/registry-adopt.test.ts` pins first-class adoption, existing-output append continuity, metadata/status transitions, output caps, exactly-once terminal publication/notification, stop/stop-all, and shutdown refusal.
- `tests/component/adopted-task-registry-ui.test.ts` and adopted-task cases in `tests/component/background-tasks-manager.test.ts` prove ordinary snapshot/dock/detail/log/history/kill behavior without source-specific UI routing.
- `tests/sdk/foreground-bash-sdk.test.ts` loads the shipped entrypoint and exercises the real override/shortcut, fast path, total-runtime auto-handoff, concurrent manual handoff, post-handoff abort ownership, shutdown cleanup, and print-mode non-backgrounding.
- `tests/pty/foreground-bash-pty.test.ts` sends real ASCII STX for `Ctrl+B`, covers automatic handoff without a key, idle no-op, and editor/Escape input after ownership transfer.
- `tests/tmux/foreground-bash-tmux.test.ts` drives the real Pi TUI through the reusable `tests/helpers/tmux-tui-harness.ts` on an isolated 80x24 pane. It sends `C-b` only after the running sentinel, expands and verifies the task receipt/id, proves `/jobs` and editor usability while the adopted process continues, requires exactly one completion notification, covers public `timeout:1` without a keypress, verifies the documented conflict with Pi's default cursor-left binding, idle no-task behavior, and subsequent editor usability, and proves `Escape` plus editor input cannot kill the adopted process.
- `tests/scripted-provider/foreground-bash-follow-up.test.ts` proves the early bash receipt precedes terminal completion, no status/log polling occurs, and registry completion starts exactly one follow-up agent run.
- `tests/windows/adopted-task-kill-routing.test.ts` covers mocked Windows terminate/force/stop-all/output-cap/missing-PID routing with no root-only fallback. `tests/windows/windows-integration.test.ts` proves `taskkill /T /F` removes a real adopted parent and descendant on Windows.
...
- foreground bash fallback: shipped `bash` override registration, fast foreground success/failure, output capture and bounded rendering, 2-second streaming/hint behavior, `Ctrl+B` manual handoff of the newest interactive command, 120-second spawn-to-handoff default and public timeout override, completion-at-boundary and duplicate-event races, pre-spawn sleep guard, non-interactive wait-to-completion behavior, abort ownership before/after handoff, adoption failure cleanup, and idle-shortcut no-op
...
- shortcut/UI: component coverage for focused dock list/detail/key handling, adopted-task snapshot/log/kill/history parity, detail output-tail scrolling (arrow/page scroll, follow-pause-on-scroll, `lines X–Y of N` position indicator, resume-follow-at-bottom, and no-scroll when output fits), empty/history/unread states, paging, close aliases, stop/stop-all/rerun/path actions, missing output files; SDK coverage for `Ctrl+B` registration/live handoff, explicit `/bg-clear` clearing of every finished state, `/bg-clear` footer hinting from hidden failed/killed notices, optional `Ctrl+Alt+C` fallback shortcut registration, running/done-only count projection, hidden-only status clearing, and focused-label projection; RPC coverage that the same count projection and `/bg-clear` clear path work through `setStatus`; tmux coverage for the foreground-bash shortcut in a real non-skipping Pi TUI; and PTY coverage for real `Ctrl+B`, `/tasks`, `/bg-tasks`, real `Shift+Down`, arrows, page keys, detail/back/history/stop/stop-all/rerun/path/close, failed unread badges, and running/completed/failed/killed rerun paths
- runtime files: output and metadata files under `.pi/tasks/`, including continuous `foreground-<sequence>-<tool-call-id>.output` ownership across adoption and task-id metadata in the registry runtime directory; Fusion private `.pi/fusion/<session-id>-<pid>/<run-id>/` artifacts plus global `fusion-models.json`, persisted `isAgent` classification, task-owned context-window telemetry snapshots, cumulative background Pi-agent token usage, tool-use counts, agent model identifier (preferring the fully-qualified `provider/model` form), explicit `isAgent:true` telemetry wrapping for background `pi` agents, `isAgent:false` non-wrapping for scripts, attested Pi flat siblings (`.pi-events.jsonl`, `.stderr`, `.pi-telemetry-wrapper.cjs`, `.attestation.json`), real child `pi --mode json` tool-event parsing for background-agent telemetry, split/large telemetry ingestion, metadata after completion/failure, Fusion v2 compact final-only metadata and explicitly marked partial-response artifacts, and Fusion manifest token plus complete cost-component aggregates equal to the sum of successful and observed failed/cancelled attempts
- extension EventBus API: unit coverage for `pi-background-tasks:request:v1`/`response:v1` closed-frame validation, exact capability handshake, malformed payload rejection, unknown keys, unknown operations, duplicate request IDs, missing `session_start`, shutdown refusal, unsubscribe, strict terminal frame shape, response-barrier ordering, and a shutdown-concurrent gated kill drained in response → terminal → service-close order exactly once; registry coverage for one terminal publication after durable metadata, loud single-attempt publication failure with no retry timer, and no pruning before phase-B settlement. SDK coverage uses a shared real `createEventBus()`, starts `printf api-ok`, reads bounded logs, observes one terminal event after the run response, and kills a real sleep task without model/provider calls
- attested Pi producer: unit/SDK coverage for 128-bit attested task ids, exact direct argv/cwd, explicit package attribution on Anthropic routes only, ModelRegistry OAuth observation without secrets, raw Pi session/message events, separate stderr, prompt/report/source hashes, authority start/finish commit/tree/clean checks, atomic metadata serialization, completion visibility only after the sidecar is durable, malformed event rejection, deterministic injected final durable-write failure with coherent failed metadata/phase-A release/loud owned rejection and no `unhandledRejection`, and no attestation sidecar for ordinary tasks
...
- safety: kill, already-finished kill failure, timeout failure, spawn failure, low output-cap failure, adopted-task output-cap termination, phase-A cancellation timeout isolation from deliberately gated phase-B notification persistence, synchronous managed cancellation under a 100 ms phase-A budget, explicit/shared finalization waits, immediate direct-close teardown, terminal-task finalization during session shutdown, mixed direct/adopted multi-task shutdown cleanup, retryable timed-out POSIX and failed Windows physical stop attempts with immutable first intent and concurrent promise sharing, retriable timed-out post-spawn startup cleanup, POSIX process-group kill fallback, Windows adopted and directly spawned `taskkill /T` then `/T /F` tree termination with shared soft attempts, soft-abort-on-force, exit-128 race tolerance, loud force failures, no root-only fallback, SIGKILL escalation that terminates instead of re-arming (a SIGKILL never schedules a further escalation, and concurrent stop requests share exactly one escalation timer that is cleared on finalize), duplicate finalization/notification races, metadata/notification failure handling, and pruning
- agent loop: deterministic scripted-provider coverage against `extensions/background-tasks.ts` for foreground bash auto-handoff (early receipt, no polling, one durable terminal notification, exactly one follow-up) and actual event-driven `bg_run` behavior. The provider observes the effective system prompt, public tool descriptions, and real launch receipt and deliberately emits the pre-fix `bg_status` poll if any contract layer is absent; the passing path proves one launch, no sleep/status/log polling, one durable terminal notification, and exactly one follow-up turn. It also covers notification-only `triggerOnCompletion:false`, `/bg` display-only behavior, `notifyOnCompletion:false`, failed-task notification error fields, and parent-model `fusion_reason` tool use followed by normal parent response
...
`test:tmux` uses `/usr/bin/tmux` directly; missing tmux is a hard failure, not an Expect fallback. Every case creates a unique dedicated tmux server/session plus isolated project, `HOME`, Pi agent directory, Pi session directory, and tmux socket directory. The harness starts an 80x24 pane with `tmux new-session -d`, launches Pi via `tmux send-keys`, polls `tmux capture-pane -p`, injects special keys (`C-b`, `C-o`, and `Escape`) directly, and always terminates through `tmux kill-session`. It enables `extended-keys on`; it intentionally does not set `extended-keys-format csi-u`, which requires tmux 3.5 while the current gate host runs tmux 3.2a.

The foreground gate loads the real `extensions/background-tasks.ts` entrypoint with the deterministic scripted provider. Unlike the Expect lane, these four tmux cases must execute with zero skips on the supported host.
...
- Foreground bash coverage sends real ASCII STX (`Ctrl+B`) only after a running sentinel, observes manual and automatic adoption plus terminal completion, proves idle `Ctrl+B` is harmless, and proves Escape/editor input after handoff does not kill the registry-owned child.
