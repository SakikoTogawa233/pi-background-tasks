...
During finalization, the runtime flushes wrapped-agent output, ends and waits for the output stream to finish/close, writes terminal metadata through the durable metadata path, initiates terminal EventBus publication, sends the completion notification when enabled and not shutting down, persists notification state, prunes old finished tasks, and only then resolves task-end waiters. Consequently, `stopTask` completion permits immediate teardown of the task directory and observes final notification metadata. Actual EventBus emission may wait behind the request-response publication gate and therefore may occur after the completion notification; it still occurs only after stream close and terminal metadata. A kill-response gate delays only terminal EventBus publication, not notification finalization, so waiting for `stopTask` cannot deadlock the response that releases that gate. The registry calls a historically named `closeAndFsyncOutputStream()` helper, but its current implementation ends and observes the stream rather than issuing `fsync` for ordinary `.output`; durable terminal truth refers to the metadata-backed status, not a stronger crash-durability guarantee for every output byte.
