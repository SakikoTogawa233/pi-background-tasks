...
The registry exposes two explicit lifecycle phases. **Phase A, durable terminal transition**, means the child or managed workflow ended, wrapped-agent output was flushed, the output stream closed, terminal metadata was written through the durable metadata path, and `task.status` became terminal. Cancellation waiters resolve at this boundary, so `stopWaitMs` bounds process/workflow termination and phase-A durability only. **Phase B, full finalization settlement**, then initiates ordered terminal EventBus publication, delivers the completion notification when applicable, and persists final `notified` metadata. Every task stores one shared phase-B promise; duplicate close/error events reuse it. A separate settled flag becomes true on either fulfillment or loud rejection. Pruning additionally requires successful terminal EventBus publication, and publication settlement reruns retention; a publication still gated or rejected therefore remains resolvable and diagnosable. `waitForFinalization(task)` exposes the shared promise and fails loudly if the task has not entered finalization.

`stopTask` refuses a competing user or shutdown stop once a natural close has claimed the shared finalizer, before changing kill classification/error state or signaling the child. While one physical stop attempt is in flight, every concurrent user/shutdown caller receives that exact promise and no duplicate signal sequence is launched. The first stop intent permanently owns `killKind` and the reason. If an attempt rejects while the task remains running, only its in-flight ownership is cleared: a later caller, especially session shutdown, may launch a fresh physical termination attempt without relabeling the task. POSIX retries may resend and escalate signals. Windows retires the prior attempt's helpers, abort controllers, and timer before creating fresh terminate/force state; recorded soft/force failures remain visible. Once finalization or terminal state has been claimed, no retry starts. Otherwise the stop requires phase A within `stopWaitMs`, then awaits phase B without charging phase-B notification/filesystem latency against the cancellation timeout. Its return therefore still permits immediate recursive teardown of the task directory and observes final notification metadata.

A registered launch failure before spawn is finalized as a coherent failed task. Once spawn returns, child `error`/`close` ownership is attached before any metadata await. Any later startup failure preserves its original error, uses the shared stop path, and keeps launch ownership until child close, phase A, and phase B settle. If that cleanup attempt times out while the child is still running, launch ownership rejects loudly and a later shutdown snapshot can retry physical termination; finalization still reports the original startup error. Session shutdown synchronously closes registry launch admission and EventBus request intake, drains every task/managed/delegate/attested launch admitted before that boundary, then takes and settles its task snapshots. It drains the exact in-flight request promises, settles a second snapshot, and drains actual terminal-publication promises before closing the service. An admitted launch therefore either reaches failed finalization without a child, cleans up its spawned child before launch ownership settles, or finishes registration in time for the shutdown snapshot to stop it; launches beginning after the boundary are rejected.

Actual EventBus emission may wait behind the request-response publication gate and therefore may occur after the completion notification; it still occurs only after stream close and terminal metadata. A kill-response gate delays only terminal EventBus publication, not notification finalization, so the initial task settlement lets `stopTask` return, the response release the gate, and request/publication drains finish without deadlock. Publication failure rejects loudly and has no retry timer. The registry calls a historically named `closeAndFsyncOutputStream()` helper, but its current implementation ends and observes the stream rather than issuing `fsync` for ordinary `.output`; durable terminal truth refers to the metadata-backed status, not a stronger crash-durability guarantee for every output byte.
