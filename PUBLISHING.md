...
```bash
PI_CODING_AGENT_DIR=$(mktemp -d) pi -e npm:pi-background-tasks@0.7.5 --offline --no-tools --no-session -p "/jobs"
...
```bash
PI_CODING_AGENT_DIR=$(mktemp -d) pi -e git:github.com/ismailsaleekh/pi-background-tasks@v0.7.5 --offline --no-tools --no-session -p "/jobs"
